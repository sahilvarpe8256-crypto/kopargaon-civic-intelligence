...
import { saveClusterDecision, isClusterMerged } from '../../services/reportStorage';
...
export default function DuplicateGroup({ 
  ...
  const [isMerged, setIsMerged] = useState(isClusterMerged(clusterId));
  const [showConfirmModal, setShowConfirmModal] = useState(false);
  const [showAllReports, setShowAllReports] = useState(false);

  const handleConfirmMerge = () => {
    const repId = report.report_id || report.reportId || report.id;
    saveClusterDecision(clusterId, 'CONFIRMED', 'Consolidated via Duplicate Intelligence');
    setIsMerged(true);
