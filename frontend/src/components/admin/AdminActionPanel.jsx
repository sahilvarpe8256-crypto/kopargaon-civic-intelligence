...
import { updateReportStatus, assignReportTeam, updateReportPriority } from '../../services/reportStorage';
...
export default function AdminActionPanel({ 
  ...
  const handleStatusChange = async (status, label) => {
    ...
    try {
      updateReportStatus(reportId, status, `Officer Decision: ${label}`);
  ...
  const handleConfirmAssign = async () => {
    setIsProcessing(true);
    try {
      assignReportTeam(reportId, selectedTeam);
  ...
  const handleConfirmPriority = async () => {
    setIsProcessing(true);
    try {
      updateReportPriority(reportId, selectedPriority);
